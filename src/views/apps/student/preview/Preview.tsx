...
import PreviewCard from 'src/views/apps/student/preview/PreviewCard'
import PreviewActions from 'src/views/apps/student/preview/PreviewActions'
...
const InvoicePreview = ({ id }: propsType) => {
  ...
  const getStudentDetailById = async () => {
    const response = await AggregatorService.getStudentDetail(id)
  ...
  return !!data ? (
    <Grid container columnSpacing={6} rowGap={2}>
            ...
            <Grid container>
              <Grid item xs={12}>
                <Typography className='preview page-header'>
                  ...
                  <Chip label={studentApplicationAllStatus[data?.application?.status]} color='warning' />
                </Typography>
              </Grid>
              <Grid item xs={12}>
      ...
      <Grid item xl={9} md={9} xs={12}>
        <PreviewCard id={id} getStudentDetailById={getStudentDetailById} studentData={data} />
      </Grid>
      <Grid item xl={3} md={3} xs={12}>
        <Box className='sticky-sidebar'>
          <PreviewActions data={data} getStudentDetailById={getStudentDetailById} />
        </Box>
      </Grid>
    </Grid>
  ) : (
