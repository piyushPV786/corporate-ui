...
import { IEnrollRowContent } from 'src/context/common'
import { Key, useState } from 'react'

interface CellType {
  row: IEnrollRowContent
}
interface IStudentList {
  responseData: IEnrollRowContent[]
...
const StudentList = ({ responseData, count }: IStudentList) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => {
        return <Typography variant='body2'>{row?.programDetails?.name}</Typography>
      }
    },
    {
      flex: 0.2,
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => {
        const courses = row?.intakeData?.intakeCoursesCourse
        ...
        return (
            ...
            title={
              <>
                <ol style={{ paddingLeft: 10 }}>
                  {courses &&
                    courses?.map((item: { course: { name: any }; batchCode: any }, index: Key | null | undefined) => (
          ...
          >
            <Typography sx={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} variant='body2'>
              {courses
                ?.map((item: { course: { name: any }; batchCode: any }) => `${item?.course?.name} - ${item?.batchCode}`)
                .join(',')}
            </Typography>
          </Tooltip>
        )
